'use strict';
// All high-level verbs the agent can run. act(verb, args) dispatches to the
// handlers below. Multi-step helpers (chests, smelting, building, farming,
// combat) sit just above the switch so related logic is easy to find.
const Vec3 = require('vec3');
const { goals } = require('mineflayer-pathfinder');
const { CFG, DATA } = require('./config');
const { state, sleep } = require('./state');
const H = require('./helpers');

const owner = CFG.owner;
const { CROP_MATURE_AGE, CROP_TO_SEED, ANIMAL_FOOD, FOOD_ANIMALS, RAW_FOODS, ORE_KEYWORDS, TOOL_TIERS } = DATA;

// ── multi-step action helpers ────────────────────────────────────────────────
async function findInChests(name, withdrawCount) {
  const bot = state.bot;
  const ids = H.chestBlockIds();
...
async function ensureCraftingTable() {
  const bot = state.bot;
  let table = bot.findBlock({ matching: H.idsForNames(['crafting_table']), maxDistance: 6 });
  if (table) return table;
  let tableItem = H.findInventory('crafting_table');
  if (!tableItem) {
    ...
    tableItem = H.findInventory('crafting_table');
  }
  ...
  return bot.findBlock({ matching: H.idsForNames(['crafting_table']), maxDistance: 4 });
}

async function smeltInput(inputName, fuelName, count) {
  const bot = state.bot;
  const fb = bot.findBlock({ matching: H.idsForNames(['furnace', 'blast_furnace', 'smoker']), maxDistance: 16 });
  ...
  const input = H.findInventory(inputName);
  ...
  try {
    ...
    const fuel = fuelName ? H.findInventory(fuelName) : H.findInventory('coal', 'charcoal', 'coal_block');
    let fueled = false;
    if (fuel) {
...
async function placeBlockAt(x, y, z, itemKey) {
  ...
  if (!H.findInventory(itemKey)) return false;
  ...
  for (const [dx, dy, dz] of refs) {
    ...
    if (ref && ref.name !== 'air' && ref.boundingBox === 'block') {
      try {
        ...
        const item = H.findInventory(itemKey);
...
async function buildHouse(args) {
  ...
  const mat = matName ? H.findInventory(matName)
    : (H.findInventory('planks') || H.findInventory('cobblestone') || H.findInventory('stone_brick')
      || H.findInventory('stone') || H.findInventory('dirt'));
...
async function ensureFarmWater() {
  const bot = state.bot;
  const bucket = H.findInventory('water_bucket');
  if (!bucket) return false;
  if (bot.findBlock({ matching: H.idsForNames(['water']), maxDistance: 6 })) return true;
  const soil = bot.findBlock({ matching: H.idsForNames(['farmland', 'dirt', 'grass_block']), maxDistance: 6 });
...
async function placeWaterAt(cellPos) {
  const bot = state.bot;
  const b = H.findInventory('water_bucket');
  if (!b) return false;
  try {
...
async function equipBestWeapon() {
  ...
  const weapon = bot.inventory.items()
    ...
    .sort((a, b) => H.armorTier(b.name) - H.armorTier(a.name))[0];
  if (weapon) {
...
async function defend(seconds) {
  ...
  let hits = 0;
  while (Date.now() < end) {
    const target = H.nearestHostile(8);
    if (!target) break;
    try {
...
async function act(verb, args) {
  const bot = state.bot;
  if (!bot || !state.connected) return { ok: false, message: 'not connected' };
  args = args || {};
  try {
    switch (verb) {
      case 'say':
      ...
      case 'bring': {
        ...
        if (H.countInInventory(name) <= 0) {
          await findInChests(name, count !== null ? count : 1e9);
        }
        if (H.countInInventory(name) <= 0) return { ok: false, message: `couldn't find any ${name}` };
        const arrive = await comeTo(args.player);
        if (!arrive.ok) return arrive;
        const item = H.resolveItem(name);
        try {
          const ent = H.playerEntity(args.player);
      ...
      case 'punch': {
        const target = H.entityByName(args.target || args.name || args.player);
      ...
      case 'find_ores': {
        let name = H.aliasName(args.name || args.block || args.item || '')
        ...
        for (const pos of positions) {
          ...
          if (!b) continue;
          if (exposedOnly && !H.isExposed(b)) continue;
      ...
      // ── farming ──────────────────────────────────────────────────────────
      case 'till': {
        const hoe = H.findInventory('_hoe');
        ...
        const ids = H.idsForNames(['dirt', 'grass_block', 'coarse_dirt', 'rooted_dirt', 'dirt_path']);
        ...
        let watered = '';
        if (tilled > 0 && H.findInventory('water_bucket')
            && !bot.findBlock({ matching: H.idsForNames(['water']), maxDistance: 6 })) {
      ...
      case 'irrigate': {
        if (!H.findInventory('water_bucket')) {
          return { ok: false, message: 'no water bucket — fill one with fill_bucket at a lake/well' };
        }
        if (bot.findBlock({ matching: H.idsForNames(['water']), maxDistance: 6 })) {
      ...
      case 'fill_bucket': {
        ...
        const water = bot.findBlock({ matching: H.idsForNames(['water']), maxDistance: 24 });
      ...
      case 'set_home': {
        ...
        state.homePos = { x: p.x, y: p.y, z: p.z };
        return { ok: true, message: `home set to ${state.homePos.x},${state.homePos.y},${state.homePos.z}` };
      }

      case 'go_home': {
        if (!state.homePos) return { ok: false, message: 'no home set — use set_home or MC_HOME=x,y,z' };
        try {
          await bot.pathfinder.goto(new goals.GoalNear(state.homePos.x, state.homePos.y, state.homePos.z, 1));
          return { ok: true, message: `back home at ${state.homePos.x},${state.homePos.y},${state.homePos.z}` };
        } catch (e) {
      ...
      case 'plant': {
        ...
        const seed = H.findInventory(seedName);
        if (!seed) return { ok: false, message: `no ${seedName} in inventory` };
        const radius = Math.max(1, Math.min(8, Number(args.radius) || 4));
        const max = Math.max(1, Math.min(32, Number(args.count) || 8));
        const positions = bot.findBlocks({ matching: H.idsForNames(['farmland']), maxDistance: radius + 2, count: 96 });
        let planted = 0;
        for (const p of positions) {
      ...
      case 'harvest': {
        ...
        const positions = bot.findBlocks({ matching: H.idsForNames(cropNames), maxDistance: radius + 2, count: 256 });
        const replant = args.replant !== false;
        let harvested = 0;
        for (const p of positions) {
          if (harvested >= max) break;
          const block = bot.blockAt(p);
          if (!H.isMatureCrop(block)) continue;
          try {
            await bot.pathfinder.goto(new goals.GoalNear(p.x, p.y, p.z, 1));
            ...
            if (replant) {
              ...
              const seed = seedName && H.findInventory(seedName);
      ...
      case 'bonemeal': {
        const bm = H.findInventory('bone_meal');
        ...
        for (const p of positions) {
          if (used >= max) break;
          const block = bot.blockAt(p);
          if (block && CROP_MATURE_AGE[block.name] !== undefined && H.isMatureCrop(block)) continue;
          try {
            await bot.pathfinder.goto(new goals.GoalNear(p.x, p.y, p.z, 2));
      ...
      case 'plant_tree': {
        ...
        const sap = H.findInventory(sapName + (sapName.includes('sapling') ? '' : '_sapling')) || H.findInventory('sapling');
        if (!sap) return { ok: false, message: 'no sapling in inventory' };
        const radius = Math.max(1, Math.min(8, Number(args.radius) || 4));
        const max = Math.max(1, Math.min(16, Number(args.count) || 3));
        const ids = H.idsForNames(['grass_block', 'dirt', 'podzol', 'coarse_dirt', 'rooted_dirt']);
        const positions = bot.findBlocks({ matching: ids, maxDistance: radius + 2, count: 96 });
        let planted = 0;
        for (const p of positions) {
          if (planted >= max) break;
      ...
      case 'explore': {
        ...
        else { dx = -Math.sin(bot.entity.yaw); dz = Math.cos(bot.entity.yaw); }
      ...
      case 'find_villagers': {
        ...
        const bell = bot.findBlock({ matching: H.idsForNames(['bell']), maxDistance: 64 });
        let message;
        if (villagers.length) {
      ...
      case 'list_trades': {
        const v = H.nearestVillager(args.range || 16);
      ...
      case 'retaliate': {
        ...
        while (Date.now() < end) {
          ...
          if (!target) target = H.nearestHostile(9);
          if (!target) break;
      ...
      case 'collect': {
        ...
        if (args.x !== undefined && args.z !== undefined) {
          ...
          const name = H.aliasName(args.name || args.block);
      ...
      case 'equip': {
        ...
        const dest = String(args.where || H.armorSlotForItem(item.name) || 'hand');
        await bot.equip(item, dest);
        return { ok: true, message: `equipped ${item.name} (${dest})` };
      }

      case 'equip_armor': {
        const slots = { head: ['helmet', 'cap', 'turtle'], torso: ['chestplate'], legs: ['leggings'], feet: ['boots'] };
        const equipped = [];
        for (const [slot, kws] of Object.entries(slots)) {
          const cands = bot.inventory.items().filter((i) => kws.some((k) => i.name.includes(k)));
          if (!cands.length) continue;
          cands.sort((a, b) => H.armorTier(b.name) - H.armorTier(a.name));
      ...
      case 'eat': {
        const food = bot.inventory.items().find((i) => H.isFood(i.name));
      ...
      case 'craft': {
        ...
        if (!recipes.length) {
          return { ok: false, message: `can't craft ${name} — missing ingredients` + (table ? '' : ' or a crafting table') };
        }
        try {
      ...
      // ── fishing / breeding / tool upgrade ──────────────────────────────────
      case 'fish': {
        const rod = H.findInventory('fishing_rod');
      ...
      case 'breed': {
        ...
        const food = foodName && H.findInventory(foodName);
        ...
        for (const kind of toolKinds) {
          ...
          const bestOwned = owned.length
            ? Math.min(...owned.map((i) => {
              const idx = TOOL_TIERS.indexOf(H.toolTier(i.name));
              return idx === -1 ? 99 : idx;
            }))
            : 99;
