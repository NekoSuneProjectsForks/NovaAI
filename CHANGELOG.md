...
### Added
- Unified Python setup script (replacing the old batch scripts) and a multi-provider installer supporting multiple LLM backends with richer configuration.
- Reminders, alarms, to-do lists, shopping lists, and calendar — with alert notifications and sound.
- Radio and music playback (SoundCloud, internet radio).
- Web GUI: pywebview + Tailwind CSS frontend, loading screen with dynamic init messages, app-readiness checks, and an enhanced dashboard layout.
...
- Avatar System documentation and initial avatar support.
- Windows niceties: window icon via Win32 API, logo/branding, installer banner.

### Changed
- Improved audio device handling (host-API-aware selection, native sample-rate prioritization) and XTTS speed consistency.
- Installer GPU flow: selectable CUDA version, CUDA-enabled PyTorch by default, hardened native command execution (`Invoke-Native`).

### Fixed
- Audio error handling and resampling.
...
### Changed
- Refactored NovaAI into a modular package.
...
[1.0.0]: https://github.com/NekoSuneVR/NovaAI/compare/v0.3.0...v1.0.0
[0.3.0]: https://github.com/NekoSuneVR/NovaAI/compare/v0.1.0...v0.3.0
